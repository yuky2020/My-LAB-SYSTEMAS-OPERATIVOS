...
sys_barrier_t simulation_barrier[2];
...
void sched_terminate(int errcode)
{
	...
	if (!errcode) {
		sys_barrier_destroy(&simulation_barrier[0]);
		sys_barrier_destroy(&simulation_barrier[1]);
...
static void sched_init(struct sched_class* sc)
{
	...
	if( sys_barrier_init(&simulation_barrier[0],nr_cpus) < 0) {
		fprintf(stderr,"Can't create barrier #0\n");
		exit(1);
	}


	if( sys_barrier_init(&simulation_barrier[1],nr_cpus) < 0) {
		fprintf(stderr,"Can't create barrier #1\n");
		exit(1);
	}
...
{
	active_sched_class->enqueue_task(t,rq,runnable);
	t->on_rq=TRUE;
...
static void scheduler_tick(int cpu,int simulation_step)
{
	...
	if (active_sched_class->task_tick)
		active_sched_class->task_tick(cpu_rq);
...
static void move_one_task(runqueue_t* src_rq,int src_cpu,runqueue_t* dst_rq,int dst_cpu, int this_cpu, int simulation_step)
{
	...
	stolen_task=active_sched_class->steal_task(src_rq);
...
static void schedule(int cpu,int simulation_step)
{
	...
	if (cpu_rq->need_resched || is_idle_task(prev)) {
		...
		next=active_sched_class->pick_next_task(cpu_rq);
...
void synchronize_cpus(int this_cpu)
{
	if (nr_cpus==1) {
		...
		sys_barrier_wait(&simulation_barrier[0]);
		...
		sys_barrier_wait(&simulation_barrier[1]);
	}
}
...
void* sched_cpu(void* arg)
{
	...
	lock_rq(cpu_rq);
	cur=active_sched_class->pick_next_task(cpu_rq);
