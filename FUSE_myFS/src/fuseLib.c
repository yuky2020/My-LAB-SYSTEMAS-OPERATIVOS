...
static int my_unlink(const char *path) {
	...
	int dirIndex; // Index in directory

	// The directory exists

	dirIndex = findFileByName(&myFileSystem, (char *)path + 1);

	if(dirIndex == -1) return -ENOENT; // Doesn't exist

	// Update root folder
	
	myFileSystem.directory.files[dirIndex].freeFile = true;
	myFileSystem.directory.numFiles--; // One file less
	int idxNodoI = myFileSystem.directory.files[dirIndex].nodeIdx;

	// Update directory
	updateDirectory(&myFileSystem);

	// Fill the fields of the new inode
	NodeStruct *node = myFileSystem.nodes[dirIndex]; // Points to node structure
	int i = 0;

	for (; i < node->numBlocks; i++) {
		myFileSystem.bitMap[node->blocks[i]] = 0; // Free space for each block
	}
	...
	// Update inodes
	node->freeNode = true; // Becomes valid again
	free(node); // Free memory

	myFileSystem.nodes[idxNodoI] = NULL; //
	myFileSystem.numFreeNodes++; // One more

	updateNode(&myFileSystem, idxNodoI, node); // Update in disk

	myFileSystem.superBlock.numOfFreeBlocks -= (i - 2); // Go back two blocks (first iteration and last not processed, starts at 0)
...
//my_read function for read a file 
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	// Size: How much we want to read - Offset: Start from where
	int bytes2Read = size;

	while(bytesRead < node->fileSize && bytes2Read) {
		int i;
		...
		for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (bytesRead < size); i++) {
			buf[i] = buffer[bytesRead++];
		}
	...
	return bytesRead; // 0 - success / negative integer - encoded error
}
...
struct fuse_operations myFS_operations = {
	...
	.unlink		= my_unlink,                    // unlink/remove a file
